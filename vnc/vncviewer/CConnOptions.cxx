...
static BoolParameter autoSelect("AutoSelect", "Auto select pixel format and encoding", false);
...
static IntParameter altKeyCode("AltKeyCode", "Alternative key code set selection for OSXvnc", 0);
...
autoReconnect(::autoReconnect), altKeyCode(::altKeyCode)
{
  if (autoSelect) {
    ...
    while (!feof(f)) {
      ...
      if (line[0] == ';') {
        ...
        if (stricmp(section.buf, "Connection") == 0) {
          ...
          } else if (stricmp(name.buf, "AltKeyCode") == 0) {
            altKeyCode = atoi(value.buf);
...
void CConnOptions::writeDefaults() {
  ...
  key.setInt(_T("AltKeyCode"), altKeyCode);
}
